        ...
        public async Task<IActionResult> AddRatingByUserAsync(RatingModel rating)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(GlobalSettings.ConnectionStr))
                {
                    ...
                    string query = $"INSERT INTO Rating (ID, Comment, Star, [Image], [Time], ProductID, UserID) VALUES('{Guid.NewGuid()}', '{rating.Comment}', {rating.Star}, '{rating.Image}', '{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}', '{rating.ProductID}', '{user.Id}')";
                    conn.Execute(query);
